 ...
 *
 * This module turns the numeric settings from fart.js into a Web Audio graph.
 ...
 *
 * Flow: fart.js creates the factory during mount(), then calls start() when
...
(() => {
  ...
  function createFlatulenceFactory() {
    // Private guts — fart.js gets the remote control, not the plumbing.
    let audioContext;
    let masterGain;
    let analyserNode;
    let captureNode;
    ...
    async function startCapture(gain) {
      ...
      // Worklet lives at repo root — resolved relative to the page URL, not this script.
