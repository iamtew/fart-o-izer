...
mpc.html                            Redirect stub to mpc/ (keeps query and hash)
mpc/                                Multi Poop Composer — drums, piano roll, mixer
  index.html                        Composer page
  const.js, model.js, persist.js    Constants, song model, library / WAV export
  audio.js, ui.js, main.js          Voices, scheduler, views, boot
  chrome.css, sequencer.css         Layout, pattern bank, timeline
  drums.css, piano.css, mixer.css   View skins
...
Open [mpc/](mpc/), or the **MPC** link on the lab masthead. Old [mpc.html](mpc.html) links still redirect. Each pattern is a 4/4 loop of 1, 2, 4, or 8 bars (double extends with empty bars you can paint; half trims the end). Double-click a pattern square to rename. Sixteen sixteenths per bar; the piano roll can switch to a 32-step grid (**x2**), stretch notes past a sixteenth, lock rows to a key/scale, and preview pitches when you click a cell or the left-hand note names. Pattern buttons sit under the drum machine or piano roll; Play loops the selected pattern, or the arrangement once clips are on the timeline (Pattern / Song switch next to Play). Pads, piano keys, and row names take several fingers at once; held keys keep sounding and Rec stretches the note until you let go. **Rec** overdubs live pad hits onto the 16th grid and live piano keys onto the selected Fart/Queef track while Play runs (Stop also ends Rec). Mixer is a full-page view with no sequencer. The drum machine is synthesized, not sampled: an **808** kit and a **909** kit, eight pads each (kick, snare, clap, closed hat, open hat, low/mid/high tom). **Add Fart** and **Add Queef** create tracks you paint on the piano roll; several of each can play at once. Every drum pad and every added track gets a mixer strip (rotary gain, EQ, bipolar filter, pan, plus a fader with a post-fader meter), plus a master strip on the right. Gain is unity at noon: left is off, right is +20 dB. Filter is off at noon: left is LPF, right is HPF. Name the song in the header, **Save** it into this browser's library, and pick it later from the dropdown. **Export JSON** / **Import** share the project as a text file; **Export WAV** downloads a stereo render. Untitled songs get a generated poop-humor name first. Double-click a mixer control or BPM to restore that control. Drag a mixer knob up or down to turn it (down raises the value). Dragging a mixer knob or fader shows the new value beside it. The working song stays in this browser via `localStorage`. Sample upload is not part of this page.
...
On the MPC page, name the song (blank names get poop-humor titles). **Save** stores it in this browser; the **Library** dropdown reloads saved songs, and **Default** loads a blank F major template. Switching songs (or importing) asks to save if the current project changed. **Export JSON** downloads the project as a `.json` text file you can send; **Import** loads that file (JSON, or an old SongID blob). **Export WAV** renders the arrangement once (or the current pattern if the timeline is empty) to `{name}-{bpm}-{key}-{scale}.wav`.
